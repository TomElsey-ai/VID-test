    ...
    if (VedoExperience) {
        ...
        const imageNodeMonitor = new MutationObserver(() => {
            ...
            if (newSrc && oldSrc && newSrc !== oldSrc) {
                ...
                if(params._diff?.i?.diff?.series){ diff?.unshift(params._diff?.i?.diff?.series)};
                console.log('diff', diff);
                if (diff && player && diff.length > 0) {
                    // player.configurationEngine.changeOptionSelection('', diff);
                    let changeArray = diff.map(r => rpoChangeFormat(r));
        ...
        const initObserver = new MutationObserver(() => {
            ...
            const startPlayer = async () => {
                ...
                await player.vehicleConfiguration.changeVehicleConfiguration({changes: [{
